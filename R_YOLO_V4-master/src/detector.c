...
void validate_detector_recall(char *datacfg, char *cfgfile, char *weightfile)
{
	...
	for (i = 0; i < m; ++i) {
		...
		//*************wxq add 20200716*****************
		box_label *truthTemp = truth;
		for (int i = 0; i<num_labels; i++)
		{
			myMinAreaRect(truthTemp);
			truthTemp++;
		}
		//**********************************************
		for (k = 0; k < nboxes; ++k) {
		...
		for (j = 0; j < num_labels; ++j) {
			++total;
			box t = { truth[j].x, truth[j].y, truth[j].w, truth[j].h,truth[j].angle };
			float best_iou = 0;
			for (k = 0; k < nboxes; ++k) {
				float iou = r_box_iou(dets[k].bbox, t);
...
float validate_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net)
{
				...
				for (class_id = 0; class_id < classes; ++class_id) {
					...
					if (prob > 0) {
						...
						for (j = 0; j < num_labels; ++j)
						{
							...
							float current_iou = r_box_iou(dets[i].bbox, t);
						...
						if (truth_index > -1) {
							...
							for (j = 0; j < num_labels_dif; ++j) {
								box t = { truth_dif[j].x, truth_dif[j].y, truth_dif[j].w, truth_dif[j].h,truth_dif[j].angle };
								float current_iou = r_box_iou(dets[i].bbox, t);
	...
	printf("\n for conf_thresh = %1.3f, precision = %1.3f, recall = %1.3f, F1-score = %1.3f \n",
		thresh_calc_avg_iou, cur_precision, cur_recall, f1_score);

	printf(" for conf_thresh = %0.3f, TP = %d, FP = %d, FN = %d, average IoU = %2.3f %% \n",
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *files, float thresh,
	float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
	...
	char ext[_MAX_EXT] = { 0 };
	char filesPath[_MAX_PATH] = { 0 };
	_splitpath(files, drive, dir, fname, ext);
	strcat(filesPath, drive);
	strcat(filesPath, dir);
	...
	if ((Handle = _findfirst(files, &FileInfo)) == -1L)
	{
		printf("没有找到测试文件夹\n");
		system("pause");
	}
	else
	{
		do
		{
			...
			//wxq 20201129 add
			char drive1[_MAX_DRIVE] = { 0 };
			char dir1[_MAX_DIR] = { 0 };
			char fname1[_MAX_FNAME] = { 0 };
			char ext1[_MAX_EXT] = { 0 };
			char filePath1[_MAX_PATH] = { 0 };
			_splitpath(filename, drive1, dir1, fname1, ext1);
			strcat(filePath1, drive1);
			strcat(filePath1, dir1);
			char path_modified[_MAX_FNAME] = { "result/" };
			strcat(filePath1, path_modified);
			if (_access(filePath1, 0) == -1)
			{
				_mkdir(filePath1);
			}
			strcat(filePath1, fname1);
			save_image(im, filePath1);
