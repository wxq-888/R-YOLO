...
//********wxq add 20201202*********
float r_box_union(box a, box b)
{
	float i;
	opencvRotateIntersection(&a, &b, &i);
	float u = a.w*a.h + b.w*b.h - i;
	return u;
}
//**********************************
...
//********wxq add 20201202*********
float r_box_iou_kind(box a, box b, IOU_LOSS iou_kind)
{
	//IOU, GIOU, MSE, DIOU, CIOU
	switch (iou_kind) {
	case IOU: return r_box_iou(a, b);
	case GIOU: return r_box_giou(a, b);
	case DIOU: return r_box_diou(a, b);
	case CIOU: return r_box_ciou(a, b);
	}
	return box_iou(a, b);
}
//**********************************
...
//********wxq add 20201202*********
float r_box_iou(box a, box b)
{
	//return box_intersection(a, b)/box_union(a, b);

	float I;
	opencvRotateIntersection(&a, &b, &I);
	float U = r_box_union(a, b);
	if (I == 0 || U == 0) {
		return 0;
	}
	return I / U;
...
//********wxq add 20201202*********
float r_box_giou(box a, box b)
{
	//计算a和b的最小外接矩形

	MY_POINT ptsA[4];
	myMinAreaRect2Points(a, ptsA, 4);
	MY_POINT ptsB[4];
	myMinAreaRect2Points(b, ptsB, 4);
	...
	box bboxComb;
	myMinAreaRect2(ptsComb, &bboxComb, 8);

	float c = bboxComb.w *bboxComb.w + bboxComb.h*bboxComb.h;
	float r_iou = r_box_iou(a, b);
	if (c == 0) {
		return r_iou;
	}
	float u = r_box_union(a, b);
...
//********wxq add 20201202*********
float r_box_diounms(box a, box b, float beta1)
{
	MY_POINT ptsA[4];
	myMinAreaRect2Points(a, ptsA, 4);
	MY_POINT ptsB[4];
...
void do_nms(box *boxes, float **probs, int total, int classes, float thresh)
{
	...
	for (i = 0; i < total; ++i) {
		...
		for (j = i + 1; j < total; ++j) {
			if (r_box_iou(boxes[i], boxes[j]) > thresh) {
				for (k = 0; k < classes; ++k) {
	...
	for (k = 0; k < classes; ++k)
	{
		...
		for (i = 0; i < total; ++i)
		{
			...
			for (j = i + 1; j < total; ++j) 
			{
				...
				if (r_box_iou(a, b) > thresh && nms_kind == CORNERS_NMS)
				{
				...
				else if (r_box_diou(a, b) > thresh && nms_kind == GREEDY_NMS) 
				{
					dets[j].prob[k] = 0;
				}
				else {
					if (r_box_diounms(a, b, beta1) > thresh && nms_kind == DIOU_NMS)
					{
						dets[j].prob[k] = 0;
					}
				}
