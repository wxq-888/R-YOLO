...
box_label *read_boxes(char *filename, int *n)
{
	...
	int count = 0;
	while (fscanf(file, "%d %f %f %f %f %f %f %f %f", &id, &x0, &y0, &x1, &y1, &x2, &y2, &x3, &y3) == 9) {
		...
		boxes[count].point[0].x = x0;
		boxes[count].point[0].y = y0;
		boxes[count].point[1].x = x1;
		boxes[count].point[1].y = y1;
		boxes[count].point[2].x = x2;
		boxes[count].point[2].y = y2;
		boxes[count].point[3].x = x3;
		boxes[count].point[3].y = y3;
...
void correct_boxes(box_label *boxes, int n, float dx, float dy, float sx, float sy, int flip, int net_w, int net_h)
{
	int i;
	for (i = 0; i < n; ++i)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			//boxes[i].point[j].x = constrain(0, 1, boxes[i].point[j].x*sx - dx);
			//boxes[i].point[j].y = constrain(0, 1, boxes[i].point[j].y*sy - dy);
			//printf("%f %f\n", boxes[i].point[j].x ,boxes[i].point[j].y);

			boxes[i].point[j].x = boxes[i].point[j].x*sx - dx;
			boxes[i].point[j].y = boxes[i].point[j].y*sy - dy;
		}
		if (flip) {
			box_label swap = boxes[i];
			boxes[i].point[0].x = 1 - swap.point[0].x;
			boxes[i].point[0].y = swap.point[0].y;
			boxes[i].point[1].x = 1 - swap.point[1].x;
			boxes[i].point[1].y = swap.point[1].y;
			boxes[i].point[2].x = 1 - swap.point[2].x;
			boxes[i].point[2].y = swap.point[2].y;
			boxes[i].point[3].x = 1 - swap.point[3].x;
			boxes[i].point[3].y = swap.point[3].y;
		}

		myMinAreaRect(&boxes[i]);
		...
		imgRect.h = 0.9999999;
		imgRect.angle = 0;
		getRotatedRectFromIntersectionConvexPolygon(&imgRect, &boxes[i], &boxes[i]);
...
void fill_truth_region(char *path, float *truth, int classes, int num_boxes, int flip, float dx, float dy, float sx, float sy)
{
	...
	box_label *boxes = read_boxes(labelpath, &count);
	randomize_boxes(boxes, count);
	correct_boxes(boxes, count, dx, dy, sx, sy, flip, 0, 0);//need modified
	float x, y, w, h;
	int id;
	int i;
...
int fill_truth_detection(const char *path, int num_boxes, int truth_size, float *truth, int classes, int flip, float dx, float dy, float sx, float sy,
	int net_w, int net_h, int swidth, int sheight)
{
	...
	randomize_boxes(boxes, count);
	correct_boxes(boxes, count, dx, dy, sx, sy, flip, net_w, net_h);
	if (count > num_boxes) count = num_boxes;
	float x, y, w, h;
	float angle;//wxq add 20200621
	int id;
	int sub = 0;
	...
	for (i = 0; i < count; ++i)
	{
		...
		w = boxes[i].w;
		h = boxes[i].h;
		angle = boxes[i].angle;//wxq add 20200621
		id = boxes[i].id;
		...
		truth[(i - sub)*truth_size + 4] = angle;
		truth[(i - sub)*truth_size + 5] = id;
	...
	int net_w, int net_h, int mosaic_bound)
{
	...
	for (t = count_new_truth; t < boxes; ++t)
	{
		...
		a.w = old_truth_ptr[2];
		a.h = old_truth_ptr[3];
		a.angle = old_truth_ptr[4];

		myMinAreaRect2Points2(&a);
		...
		if (i_mixup == 0)
		{
			for (int i = 0; i < 4; i++)
			{
				a.point[i].x = a.point[i].x - (float)(w - cut_x - right_shift) / w;
				a.point[i].y = a.point[i].y - (float)(h - cut_y - bot_shift) / h;
			}
		}
		if (i_mixup == 1)
		{
			for (int i = 0; i < 4; i++)
			{
				a.point[i].x = a.point[i].x + (float)(cut_x - left_shift) / w;
				a.point[i].y = a.point[i].y - (float)(h - cut_y - bot_shift) / h;
			}
		}
		if (i_mixup == 2)
		{
			...
			{
				a.point[i].x = a.point[i].x - (float)(w - cut_x - right_shift) / w;
				a.point[i].y = a.point[i].y + (float)(cut_y - top_shift) / h;
			}
		}
		if (i_mixup == 3)
		{
		...
		for (int i = 0; i < 4; i++)
		{
			a.point[i].x = a.point[i].x * w;
			a.point[i].y = a.point[i].y * h;
		}
		myMinAreaRect(&a);
		...
		getRotatedRectFromIntersectionConvexPolygon(&b, &a, &a);
		...
		// leave only within the image
		if (a.x >= 0 && a.x < 1 && a.y >= 0 && a.y < 1 && a.w > lowest_w && a.h > lowest_h&& a.w < 1 && a.h < 1)
		{
			new_truth_ptr[0] = a.x;
			new_truth_ptr[1] = a.y;
			new_truth_ptr[2] = a.w;
			new_truth_ptr[3] = a.h;
			new_truth_ptr[4] = a.angle;
	...
	float jitter, float resize, float hue, float saturation, float exposure, int mini_batch, int track, int augment_speed, int letter_box, int mosaic_bound, int contrastive, int contrastive_jit_flip, int contrastive_color, int show_imgs)
{
	...
	for (i_mixup = 0; i_mixup <= use_mixup; i_mixup++)
	{
		...
		for (i = 0; i < n; ++i)
		{
			...
			int min_w_h = fill_truth_detection(filename, boxes, truth_size, truth, classes, flip, dx, dy, 1. / sx, 1. / sy, w, h, swidth, sheight);
			...
			if (show_imgs && i_mixup == use_mixup && saveFlag == 1)   // delete i_mixup
			{
				...
				for (t = 0; t < boxes; ++t)
				{
					...
					my_draw_box_width_height(&tmp_ai, &b, buff);
				}
	...
	float jitter, float resize, float hue, float saturation, float exposure, int mini_batch, int track, int augment_speed, int letter_box, int mosaic_bound, int contrastive, int contrastive_jit_flip, int contrastive_color, int show_imgs)
{
	...
	for (i_mixup = 0; i_mixup <= mixup; i_mixup++) {
		...
		for (i = 0; i < n; ++i) {
			...
			fill_truth_detection(filename, boxes, truth_size, truth, classes, flip, dx, dy, 1. / sx, 1. / sy, w, h, ow, oh);
