...
image float_to_cow_image(int w, int h, int c, float *data, int ai)
{
	...
	for (int k = 0; k < c; k++) {
		if (ai < 10)
			sprintf(tp, "E:\\DeepLearning\\R_Data\\feature_pic\\out_000%d", ai);
		else
			sprintf(tp, "E:\\DeepLearning\\R_Data\\feature_pic\\out_00%d", ai);
		myMkdir(tp);
		if (ai < 10)
			sprintf(tp, "E:\\DeepLearning\\R_Data\\feature_txt\\out_000%d", ai);
		else
			sprintf(tp, "E:\\DeepLearning\\R_Data\\feature_txt\\out_00%d", ai);
		myMkdir(tp);
		if (ai < 10)
			sprintf(tp, "E:\\DeepLearning\\R_Data\\feature_txt\\out_000%d\\out_000%d_%d.txt", ai, ai, k);
		else
			sprintf(tp, "E:\\DeepLearning\\R_Data\\feature_txt\\out_00%d\\out_00%d_%d.txt", ai, ai, k);
