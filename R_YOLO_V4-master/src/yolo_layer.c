...
layer make_yolo_layer(int batch, int w, int h, int n, int total, int *mask, int classes, int max_boxes)
{
    ...
    l.truth_size = 4 + 2 + 1;//源码中是4+2，不知道为何是4+1，由于我们需要+1所有变成了4 + 2 + 1
    l.truths = l.max_boxes*l.truth_size;    // 90*(4 + 1);//如果影像中目标的数量超过90 该如何修改？
...
box get_yolo_box(float *x, float *biases, int n, int index, int i, int j, int lw, int lh, int w, int h, int stride, int new_coords)
{
    ...
    if (new_coords) {
		...
		float anchorAngle = 0;//归一化处理，由于标注的角度是[-90,0],设置锚框的角度是-90所以归一化之后变为-1
        b.x = (i + x[index + 0*stride]) / lw;
        b.y = (j + x[index + 1*stride]) / lh;
        b.w = exp(x[index + 2*stride]) * biases[2*n]   / w;
        b.h = exp(x[index + 3*stride]) * biases[2*n+1] / h;
		//下式中x[index + 4 * stride] 之所以是负号，是因为我们将角度通过LOGIST函数映射到（0~1）的范围内，是正号，角度应该是负值，所以前面加上负号。
		b.angle =  x[index + 4 * stride] + anchorAngle; 
		b.angle = b.angle * 90;//因为ground truth中标注的是角度，所以通过预测的和真是的求IOU时，应该也用角度，在这Opencv中的函数也是角度
...
{
    if (delta[index + 0 * stride] || delta[index + 1 * stride] || delta[index + 2 * stride] || delta[index + 3 * stride] || delta[index + 4 * stride]) {
        (*rewritten_bbox)++;
    }
    ...
    all_ious.iou = r_box_iou(pred, truth);
    all_ious.giou = r_box_giou(pred, truth);
    all_ious.diou = r_box_diou(pred, truth);
    all_ious.ciou = r_box_ciou(pred, truth);
    // avoid nan in dx_box_iou
    if (pred.w == 0) { pred.w = 1.0; }
    if (pred.h == 0) { pred.h = 1.0; }
	float anchorAngle = 0;//认为锚框的角度为-90,并进行归一化处理，由于标注的角度是[-90,0],设置锚框的角度是-90所以归一化之后变为-1
    if (iou_loss == MSE)    // old loss
    {
		...
		float ta = truth.angle / 90 - anchorAngle; //truth.angle / 90对角度进行归一化处理
        ...
        //printf(" x = %f, y = %f, w = %f, h = %f, angle = %f \n", x[index + 0 * stride], x[index + 1 * stride], x[index + 2 * stride], x[index + 3 * stride], -x[index + 4 * stride]);
		...
		//因为ta是归一化之后的角度范围为（-1~0）
		//所以x[index + 4 * stride]也是通过logistics函数映射到（0~1）的角度，相当于归一化
		//由于角度范围为（0~1）所以前面应该取负值，使范围变为（-1~0）
		delta[index + 4 * stride] += scale * (ta - x[index + 4 * stride]) * iou_normalizer; 
	}
    else {
...
static int entry_index(layer l, int batch, int location, int entry)
{
	...
	return batch*l.outputs + n*l.w*l.h*(4 + 1 + l.classes + 1 ) + entry*l.w*l.h + loc;
}
    ...
    *(l.cost) = 0;
    for (b = 0; b < l.batch; ++b) {
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    const int class_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 4 + 1 + 1);
                    const int obj_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 4 + 1);
                    ...
                    for (t = 0; t < l.max_boxes; ++t) {
                        ...
                        int class_id = state.truth[t*l.truth_size + b*l.truths + 4 + 1];
                        ...
                        float iou = r_box_iou(pred, truth);
                    ...
                    if (best_match_iou > l.ignore_thresh) {
                        ...
                        if (found_object) {
							...
							l.delta[box_index + 4 * stride] += scale * (0 - l.output[box_index + 4 * stride]);
                        }
                    }
                    if (best_iou > l.truth_thresh) {
                        ...
                        int class_id = state.truth[best_t*l.truth_size + b*l.truths + 4 + 1];
        ...
        for (t = 0; t < l.max_boxes; ++t) {
            ...
            for (n = 0; n < l.total; ++n) {
				...
				pred.angle = 0; //由于标注的角度是[-90,0],认为锚框的角度为-90,此处没有归一化，因为需要计算IOU，计算IOU需要真实的旋转角度
                float iou = r_box_iou(pred, truth_shift);
                if (iou > best_iou) {
                    best_iou = iou;
                    best_n = n;
                }
            ...
            if (mask_n >= 0) {
                ...
                const int truth_in_index = t*l.truth_size + b*l.truths + 5 +1;
                ...
                int obj_index = entry_index(l, b, mask_n*l.w*l.h + j*l.w + i, 4 + 1);
                ...
                int class_index = entry_index(l, b, mask_n*l.w*l.h + j*l.w + i, 4 + 1 + 1);
            ...
            for (n = 0; n < l.total; ++n) {
                ...
                if (mask_n >= 0 && n != best_n && l.iou_thresh < 1.0f) {
                    ...
                    pred.h = l.biases[2 * n + 1] / state.net.h;
					//******************wxq 20201202 add*******************
                    float iou = r_box_iou_kind(pred, truth_shift, l.iou_thresh_kind); // IOU, GIOU, MSE, DIOU, CIOU
                    // iou, n
        ...
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    int obj_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 4 + 1);
                    int box_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 0);
                    int class_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 4 + 1 + 1);
                    const int stride = l.w*l.h;
...
void correct_yolo_boxes(detection *dets, int n, int w, int h, int netw, int neth, int relative, int letter)
{
	...
	for (i = 0; i < n; ++i) {
		...
		box b = dets[i].bbox;
		MY_POINT pt[4];
		myMinAreaRect2Points(b, pt, 4);
		for (int j = 0; j < 4; j++)
		{

			pt[j].x = (pt[j].x - deltaw / 2. / netw) / ratiow;
			pt[j].y = (pt[j].y - deltah / 2. / neth) / ratioh;
...
int yolo_num_detections(layer l, float thresh)
{
    ...
    for(n = 0; n < l.n; ++n){
        for (i = 0; i < l.w*l.h; ++i) {
            int obj_index  = entry_index(l, 0, n*l.w*l.h + i, 4 + 1);
...
int yolo_num_detections_batch(layer l, float thresh, int batch)
{
    ...
    for (i = 0; i < l.w*l.h; ++i){
        for(n = 0; n < l.n; ++n){
            int obj_index  = entry_index(l, batch, n*l.w*l.h + i, 4 + 1);
            if(l.output[obj_index] > thresh){
                ++count;
            }
        }
        ...
        for(n = 0; n < l.n; ++n){
            ...
            if (objectness > thresh) {
                ...
                for (j = 0; j < l.classes; ++j) {
                    int class_index = entry_index(l, 0, n*l.w*l.h + i, 4 + 1 + 1 + j);
    ...
    for (i = 0; i < l.w*l.h; ++i){
        ...
        for(n = 0; n < l.n; ++n){
            ...
            if (objectness > thresh) {
                ...
                for (j = 0; j < l.classes; ++j) {
                    int class_index = entry_index(l, batch, n*l.w*l.h + i, 4 + 1 + 1 + j);
                    float prob = objectness*predictions[class_index];
                    dets[count].prob[j] = (prob > thresh) ? prob : 0;
                }
                ++count;
...
void forward_yolo_layer_gpu(const layer l, network_state state)
{
    ...
    for (b = 0; b < l.batch; ++b){
        for(n = 0; n < l.n; ++n){
			...
			////**************wxq add 20201224**************
			//index = entry_index(l, b, n*l.w*l.h, 4);//angle的index
			//activate_array_ongpu(l.output_gpu + index, l.w*l.h, LOGISTIC);//angle
			//activate_array_ongpu(l.output_gpu + index, l.w*l.h, OPPOSITE_NUMBER);
			////********************************************
			index = entry_index(l, b, n*l.w*l.h, 4 + 1);
