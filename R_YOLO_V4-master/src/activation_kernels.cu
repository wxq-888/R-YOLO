...
//*********wxq add 20201224****************
__global__ void activate_array_opposite_number_kernel(float *x, int n)
{
...
extern "C" void activate_array_ongpu(float *x, int n, ACTIVATION a)
{
    ...
    if (a == LINEAR) return;
	else if(a== OPPOSITE_NUMBER) activate_array_opposite_number_kernel << <num_blocks, BLOCK, 0, get_cuda_stream() >> >(x, n);
