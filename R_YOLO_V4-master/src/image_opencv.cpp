...
// ====================================================================
	...
	void mat_to_image2(image* srcImg, cv::Mat mat);
	...
	*/
	...
	extern "C"  void mat_to_image2(image* srcImg, cv::Mat mat)
	{
		int w = mat.cols;
		int h = mat.rows;
		int c = mat.channels();
		...
		for (int y = 0; y < h; ++y) {
			for (int k = 0; k < c; ++k) {
				for (int x = 0; x < w; ++x) {
					...
					srcImg->data[k*w*h + y*w + x] = data[y*step + x*c + k] / 255.0f;
				}
			}
		}
	}
	...
	*/
		...
		float precision, int draw_precision, char *accuracy_name, float contr_acc, int dont_show, int mjpeg_port, double time_remaining)
	{
		try {
			...
			if (k == 's' || current_batch == (max_batches - 1) || (current_batch / 100 > old_batch / 100)) {
				...
				char tmp[32] = { 0 };
				snprintf(tmp, sizeof(tmp), "%04d-%02d-%02d_%02d-%02d-%02d", 1900 + stime->tm_year, 1 + stime->tm_mon, stime->tm_mday, stime->tm_hour, stime->tm_min, stime->tm_sec);
				static std::string timeTmp = std::string(tmp);
				static std::string chartName = std::string(windows_name) + std::string("chart") + timeTmp + std::string(".png");

				save_mat_png(img, "chart.png");
				save_mat_png(img, chartName.c_str());
	...
	void myMinAreaRect(box_label* bbox)
	{
		...
		bbox->x = rRect.center.x;
		bbox->y = rRect.center.y;
		bbox->w = rRect.size.width;
		bbox->h = rRect.size.height;
		bbox->angle = rRect.angle;
	}
	void myMinAreaRect2(MY_POINT* pts, box* bbox, int n)
	{
		std::vector<cv::Point2f> points;
		for (int j = 0; j < n; j++)
		{
	...
	void myMinAreaRect2Points(box a, MY_POINT pt[4])
	{
		cv::RotatedRect rRect;
		rRect.center.x = a.x;
		rRect.center.y = a.y;
		rRect.size.width = a.w;
		rRect.size.height = a.h;
		rRect.angle = a.angle;
		cv::Point2f point[4];
		rRect.points(point);
		for (int i = 0; i < 4; i++)
	...
	void myMinAreaRect2Points2(box_label* a)
	{
		cv::RotatedRect rRect;
		rRect.center.x = a->x;
		rRect.center.y = a->y;
		...
		for (int i = 0; i < 4; i++)
		{
			...
			a->point[i].y = point[i].y;
		}
	}
	void my_draw_box_width_height(image* tmp_ai, box* ptrBBox)
	{
		cv::Mat img = image_to_mat(*tmp_ai);
		cv::RotatedRect rRect;

		rRect.center.x = ptrBBox->x*img.cols;
		rRect.center.y = ptrBBox->y*img.rows;
		rRect.size.width = ptrBBox->w *img.cols;
		rRect.size.height = ptrBBox->h*img.rows;
		rRect.angle = ptrBBox->angle;
		cv::Mat pointsTurned;
		cv::boxPoints(rRect, pointsTurned);
		std::vector<std::vector<cv::Point2i>>pointsVecTurned;
		std::vector<cv::Point2i> tempVec;
		for (int i = 0; i < pointsTurned.rows; i++)
		{
			cv::Point2i pt;
			pt.x = pointsTurned.at<float>(i, 0);
			pt.y = pointsTurned.at<float>(i, 1);
			tempVec.push_back(pt);
		}
		if (tempVec.size()==0)
		{
			printf("没有边界框");
		}
		pointsVecTurned.push_back(tempVec);
		cv::drawContours(img, pointsVecTurned, -1, cv::Scalar( 0, 255, 0), 1);
		*tmp_ai = mat_to_image(img);
	}

	void my_draw_box_width2(image* tmp_ai, box* ptrBBox)
	{
		cv::Mat img = image_to_mat(*tmp_ai);
		cv::RotatedRect rRect;
		...
		pointsVecTurned.push_back(tempVec);
		cv::drawContours(img, pointsVecTurned, -1, cv::Scalar(255, 0, 0), 2);
		*tmp_ai = mat_to_image(img);
	}
	void my_draw_box_point(image* tmp_ai, MY_POINT point[4])
	{
		std::vector<std::vector<cv::Point2i>>pointsVecTurned;
		std::vector<cv::Point2i> tempVec;
		for (int i = 0; i < 4; i++)
		{
			cv::Point2i pt;
			pt.x = point[i].x*tmp_ai->w;
			pt.y = point[i].y*tmp_ai->h;
			tempVec.push_back(pt);
		}
		cv::RotatedRect rRect;
		rRect = cv::minAreaRect(tempVec);
		cv::Point2f pts[4];
		rRect.points(pts);
		std::vector<cv::Point2i> tempVec2;
		for (int i = 0; i < 4; i++)
		{
			tempVec2.push_back(cv::Point2i(pts[i].x, pts[i].y));
		}
		cv::Mat img = image_to_mat(*tmp_ai);
		pointsVecTurned.push_back(tempVec2);
		int thickness = int(0.003*img.rows) <= 2 ? 2 : int(0.004*img.rows);
		cv::drawContours(img, pointsVecTurned, -1, cv::Scalar(0, 255, 0), thickness);
		mat_to_image2(tmp_ai,img);
	}

	void test_draw_box_width(box_label* ptrBBox, int n, char* name)
	{
		cv::Mat img = cv::Mat(cv::Size(416, 416), CV_8UC3, cv::Scalar(255, 255, 255));
		cv::RotatedRect rRect;
	...
	void opencvRotateIntersection(rotatedRect_cv* r1, rotatedRect_cv* r2, float* area)
	{

		cv::RotatedRect &cpp_r1 = *(cv::RotatedRect*)r1;
		cv::RotatedRect &cpp_r2 = *(cv::RotatedRect*)r2;
		cv::Point2f p1[4];
		cpp_r1.points(p1);
		...
		int intersectionType = cv::rotatedRectangleIntersection(cpp_r1, cpp_r2, vertices);
		if (vertices.empty())
		{
			*area = 0;
		}
		else
		{
			...
			//std::vector<cv::Point2f> vertices1;
			//std::vector<cv::Point2f> vertices2;
			//for (int i = 0; i < cnt; i++)
			//{
			...
			//	}
			//}
			//std::map<float, cv::Point2f> setV1;
			//std::map<float, cv::Point2f, std::greater<float>> setV2;
			//for (int i = 0; i < vertices1.size(); i++)
			//{
			//	cv::Point2f dalte = vertices1[i] - center;
			//	float dist = sqrt(dalte.x*dalte.x + dalte.y*dalte.y);
			//	float angle = acos(dalte.x / dist);
			//	setV1.insert(std::pair<float, cv::Point2f>(angle, vertices1[i]));
			//}
			//for (int i = 0; i < vertices2.size(); i++)
			//{
			//	cv::Point2f dalte = vertices2[i] - center;
			//	float dist = sqrt(dalte.x*dalte.x + dalte.y*dalte.y);
			//	float angle = acos(dalte.x / dist);
			//	setV2.insert(std::pair<float, cv::Point2f>(angle, vertices2[i]));
			//}
			//vertices.clear();
			//for (auto iter = setV1.begin(); iter != setV1.end(); iter++)
			//{
			//	vertices.push_back(iter->second);
			//}
			//for (auto iter = setV2.begin(); iter != setV2.end(); iter++)
			//{
			//	vertices.push_back(iter->second);
			//}
			...
			std::sort(vertices.begin(), vertices.end(), cmp_angle);
			*area = cv::contourArea(vertices);
		}
	}
	int bePointInPoly(std::vector<cv::Point2f> vertex, float testx, float testy)
	{
		int i, j, c = 0;
		...
		for (i = 0, j = nvert - 1; i < nvert; j = i++)
		{
			if (((vertex[i].y > testy) != (vertex[j].y > testy)) &&
				(testx < (vertex[j].x - vertex[i].x) * (testy - vertex[i].y) / (vertex[j].y - vertex[i].y) + vertex[i].x))
				c = !c;
		}
		return c;
	}

	int bePointInPoly2(cv::Point2f vertex[], float testx, float testy)
	{
		int i, j, c = 0;
		int nvert = 4;
		for (i = 0, j = nvert - 1; i < nvert; j = i++)
	...
	int getRotatedRectFromIntersectionConvexPolygon(box_label* r1, box_label* r2, box_label* resultPolygon)
	{
		cv::RotatedRect cpp_r1;
		cpp_r1.center.x = r1->x;
		cpp_r1.center.y = r1->y;
		cpp_r1.size.width = r1->w;
		cpp_r1.size.height = r1->h;
		cpp_r1.angle= r1->angle;
		cv::Point2f p1[4];
		cpp_r1.points(p1);


		cv::RotatedRect cpp_r2;
		cpp_r2.center.x = r2->x;
		cpp_r2.center.y = r2->y;
		cpp_r2.size.width = r2->w;
		cpp_r2.size.height = r2->h;
		cpp_r2.angle = r2->angle;
		...
		if(vertices.size()>2)
		{
			cv::RotatedRect cpp_resultPolygon = cv::minAreaRect(vertices);
			cv::Point2f pt4[4];
			cpp_resultPolygon.points(pt4);
			resultPolygon->x = cpp_resultPolygon.center.x;
			resultPolygon->y = cpp_resultPolygon.center.y;
			resultPolygon->w = cpp_resultPolygon.size.width;
			resultPolygon->h = cpp_resultPolygon.size.height;
			resultPolygon->angle = cpp_resultPolygon.angle;
			for (int k = 0; k < 4; k++)
			{
				resultPolygon->point[k].x = pt4[k].x;
				resultPolygon->point[k].y = pt4[k].y;
			}
		}
		else
		{
			resultPolygon->x = 999999;
			resultPolygon->y = 999999;
			resultPolygon->w = 999999;
			resultPolygon->h = 999999;
			resultPolygon->angle = 0;
			myMinAreaRect2Points2(resultPolygon);
			return false;
		}
		//else
		//{
		...
		//	};
		//	std::sort(vertices.begin(), vertices.end(), cmp_angle);
		//	//**********************开始找三个点的外接矩形，而且矩形在图像范围内***********************
		//	std::multimap<float, std::vector<cv::Point2f>, std::greater<float>> point4Multimap;
		//	int verticesSize = vertices.size();
		//	for (int i = 0; i < verticesSize; i++)
		//	{
		//		cv::RotatedRect rect;
		//		int index1 = i % verticesSize;
		//		int index2 = (i + 1) % verticesSize;
		//		int index3 = (i + 2) % verticesSize;

		//		cv::Point2f aPoint = cv::Point2f(vertices[index2].x - vertices[index1].x, vertices[index2].y - vertices[index1].y);
		//		cv::Point2f bPoint = cv::Point2f(vertices[index2].x - vertices[index3].x, vertices[index2].y - vertices[index3].y);

		//		//float angle123 = abs(cv::fastAtan2(aPoint.y, aPoint.x) - cv::fastAtan2(bPoint.y, bPoint.x));
		//		//if (abs(angle123 - 90) > 0.1)
		//		//{
		//		//	continue;
		//		//}
		//		cv::Point2f ptIndex4;
		//		ptIndex4.x = vertices[index1].x + vertices[index3].x - vertices[index2].x;
		//		ptIndex4.y = vertices[index1].y + vertices[index3].y - vertices[index2].y;
		//		std::vector<cv::Point2f> verticesPart;
		//		verticesPart.push_back(vertices[index1]);
		//		verticesPart.push_back(vertices[index2]);
		//		verticesPart.push_back(vertices[index3]);
		//		verticesPart.push_back(ptIndex4);
		//		rect = cv::minAreaRect(verticesPart);
		//		cv::Point2f p3[4];
		//		rect.points(p3);
		//		int inPoints = 0;
		//		for (int j = 0; j < 4; j++)
		//		{
		//			bool bIn = bePointInPoly2(p1, p3[j].x, p3[j].y);
		//			if (bIn)
		//			{
		//				inPoints++;
		//			}
		...
		//		if (inPoints==4)
		//		{
		//			float dist1 = pow(p3[0].x - p3[1].x, 2) + pow(p3[0].y - p3[1].y, 2);
		//			float dist2 = pow(p3[2].x - p3[1].x, 2) + pow(p3[2].y - p3[1].y, 2);
		//			float whRatio = dist1 > dist2 ? dist2 / dist1 : dist1 / dist2;
		//			//std::vector<cv::Point2f>vInt(p3, p3 + 4);
		//			//float area = cv::contourArea(vInt);
		//			point4Multimap.insert(std::pair<float, std::vector<cv::Point2f> >(whRatio, verticesPart));
		//		}
		//		else
		//		{
		...
		//	}
		//	if (point4Multimap.size() != 0)
		//	{
		//		auto iter = point4Multimap.begin();
		//		std::vector<cv::Point2f> verticesPart =iter->second;
		//		cv::RotatedRect cpp_resultPolygon = cv::minAreaRect(verticesPart);
		//		cv::Point2f pt4[4];
		//		cpp_resultPolygon.points(pt4);
		//		resultPolygon->x = cpp_resultPolygon.center.x;
		//		resultPolygon->y = cpp_resultPolygon.center.y;
